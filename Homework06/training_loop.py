...
def training_cifar(training_data, test_data, model,epochs,plot = False, save = False):
    ...
    time_string = "original_3"
    
    train_file_path = f"logs/{time_string}/train"
    test_file_path = f"logs/{time_string}/test"
    ...
    if save:
        data_frame.to_csv(f"csvs/{time_string}")
    ...
    if plot:
        ...
        plt.savefig(f"Plots/{time_string}.png")
        plt.show()