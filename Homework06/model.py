...
class MyCNNBlock(tf.keras.layers.Layer):
    ...
    def __init__(self,layers,filters,global_pool = False,mode = False,normalization = False, reg = None):
        """ Constructor 
...
class MyCNN(tf.keras.Model):
    ...
    def __init__(self, optimizer,output_units : int = 10, mode = None,normalization = False,dropout = 0,regularizer = 0):
        """ Constructor 
        ...
        # architecture
        self.reg = tf.keras.regularizers.L2(regularizer)
        self.dropout = tf.keras.layers.Dropout(rate=dropout) if dropout >= 0 else None
        self.block1 = MyCNNBlock(layers = 2,filters = 24,mode = mode,normalization = normalization, reg = self.reg)
        self.block2 = MyCNNBlock(layers = 2,filters = 48,mode = mode,normalization = normalization, reg = self.reg)
        self.block3 = MyCNNBlock(layers = 2,filters = 96,global_pool=True, mode = mode,normalization = normalization, reg = self.reg)

        self.dense1 = tf.keras.layers.Dense(128,activation = tf.nn.relu, kernel_regularizer = self.reg)
